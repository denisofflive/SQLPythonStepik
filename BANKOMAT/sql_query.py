...
class SQL_atm:
    ...
    def input_operation(number_card):
        while True:
            ...
            # Если операция 4 - Завершить работу
                ...
                SQL_atm.transfer_money(number_card, recipient_card, int(amount))
    ...
    @staticmethod
    def transfer_money(sender_card, recipient_card, amount):
        try:
            with sqlite3.connect("atm.db") as db:
                cur = db.cursor()
                ...
                if amount > sender_balance:
                    print("На вашей карте недостаточно средств для перевода.")
                    return False
                ...
                SQL_atm.report_operation_2(now_data, sender_card, 3, amount, recipient_card)
    ...
    @staticmethod
    def report_operation_2(now_date, number_card, type_operation, amount, payee):
        try:
            user_data = [
                (now_date, number_card, type_operation, amount, payee)
            ]
